...
def mask_experiment(scale_factor):
    ...
    mask_input_size = [
        4 * x for x in predictor.model.prompt_encoder.image_embedding_size
    ]
    torch.randn(1, *mask_input_size, dtype=torch.float32)
    prefix = os.path.dirname(data_path[0])

    out_dir = os.path.join(f"{prefix}-video-segment-with-gt")
    try:
    ...
    point, label = np.array([[400, 300]]), np.array([1.0])
    predict_masks = None
    ...
    for path, mask_path in tqdm(
        ...
        r.add(img, (mask == chosen_class).astype(np.float16), [point, label], "GT")
        r.add_multiple(
            [
                {
                    "img": None,
