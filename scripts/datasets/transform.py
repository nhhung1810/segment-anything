...
from monai.transforms import MapTransform, Compose, NormalizeIntensityd
import numpy as np
...
class LoadImageAndResize3D(MapTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            ...
            if key == "label":
                resize_img = self._resize_image(
                    image, self.size, resamplemethod=sitk.sitkNearestNeighbor
                )
            else:
                resize_img = self._resize_image(
                    image, self.size, resamplemethod=sitk.sitkLinear
                )
...
class PercentileClip(MapTransform):
    ...
    def __init__(
        self, keys, min_pct: int, max_pct: int, allow_missing_keys: bool = False
    ) -> None:
        super().__init__(keys, allow_missing_keys=allow_missing_keys)
...
class IntensityClip(MapTransform):
    ...
    def __init__(
        self, keys, min_value: int, max_value: int, allow_missing_keys: bool = False
    ) -> None:
        super().__init__(keys, allow_missing_keys=allow_missing_keys)
