...
def post_process(pred: np.ndarray, gt_file: str, out_dir: str):
    raw_image_dict = load_ct_info(gt_file)
    pred_image_dict = {"mask": pred.transpose(2, 0, 1)}
...
def post_process_file(pred_file, gt_file, out_dir):
    os.makedirs(out_dir, exist_ok=True)
    pred = np.load(pred_file)

    post_process(pred, gt_file, out_dir)
...
if __name__ == "__main__":
    ...
    PRED_MASK_PATH = "runs/sam-one-point-230501-012024/inference/FLARE22_Tr_0008.npy"

    post_process_file(
        pred_file=PRED_MASK_PATH,
        gt_file=TEST_MASK_PATH,
        out_dir=os.path.dirname(PRED_MASK_PATH),
    )
    pass
