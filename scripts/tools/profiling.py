...
def torch_profiler_time_cpu_gpu(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        format_time = lambda time_us: '{:.3f}us'.format(time_us)
...
class GPUProfiler:
    def __init__(self, gpu_index, gpu_memory_txt_path) -> None:
        self.gpu_index = gpu_index
        self.gpu_memory_txt_path = gpu_memory_txt_path
        with open(self.gpu_memory_txt_path, 'w'): pass
        self.p = None
        pass

    def __enter__(self):
        self.p = Process(target=get_gpu_memory_usage, args=(0.1, True, self.gpu_index, self.gpu_memory_txt_path))
        self.p.daemon = True
        self.p.start()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.total_time = (end - self.start) * 1e6
        print(f"Ellapsed duration: {self.total_time}", flush=True)
        return True
    ...
    def inspect(self):
        ...
        self.total_time = (end - self.start) * 1e6
        print(f"Current duration: {self.total_time}", flush=True)
        pass

    def get_time(self):
        end = time.perf_counter()
        total_time = (end - self.start) * 1e6
        return total_time
...
if __name__ == '__main__':
    ...
    # func()
    gpu_index = 0
    with GPUProfiler(gpu_index=gpu_index, gpu_memory_txt_path='gpu_tmp.txt') as profiler:
        for _ in range(10 * 10):
            a = torch.zeros(512, 512).to(f'cuda:{gpu_index}')
            time.sleep(1)
            profiler.inspect()